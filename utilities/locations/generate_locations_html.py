...
def _build_world_map_html(
    ...
    if locations_geojson:
        ...
        layers_js += (
            ...
            '(function(){'
            'function _mk(name,draw){var c=document.createElement("canvas");c.width=32;c.height=32;draw(c.getContext("2d"));map.addImage(name,c);}'
            ...
            '_mk("cat-sacred-site",function(ctx){'
              ...
              'ctx.lineTo(25,29);ctx.closePath();ctx.fill();'
            '});'
            '_mk("cat-fortress",function(ctx){'
              'ctx.fillStyle="#7a1f1f";'
              'ctx.fillRect(4,15,24,13);'
              'ctx.fillRect(4,8,6,8);ctx.fillRect(13,8,6,8);ctx.fillRect(22,8,6,8);'
            '});'
            '_mk("cat-oasis",function(ctx){'
              'ctx.strokeStyle="#b8892a";ctx.lineWidth=4;ctx.lineCap="round";ctx.fillStyle="#b8892a";'
              '[[16,16,16,3],[16,16,26.9,9.5],[16,16,22.5,23.5],[16,16,9.5,23.5],[16,16,5.1,9.5]].forEach(function(p){'
            ...
            '_mk("cat-landmark",function(ctx){'
              ...
              'ctx.quadraticCurveTo(22,14,30,28);ctx.closePath();ctx.fill();'
            '});'
