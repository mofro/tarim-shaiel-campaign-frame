...
def generate_category_page(
    ...
    # Back-nav href/label for template
    if back_href_override:
        _nav_back_href  = back_href_override
        _nav_back_label = back_label_override or 'Back'
    elif parent_bucket:
        _nav_back_href  = f'category-{parent_bucket}.html'
    ...
    # Filter out group-label items (anchor=None) before passing to template sidebar
    sidebar_nav_items = [i for i in all_nav_items if i.get('anchor')]
    ...
    html = _render_page(
        ...
        back_label=_nav_back_label,
        jump_nav_items=sidebar_nav_items if _has_sidebar else [],
        sidebar_heading=cfg.get('sidebar_heading', ''),
        subcats_html=subcats_html,
        items_label=f'{escape(label)} ({count})' if count > 0 else '',
        cards_html=cards_html if count > 0 else '',
        count=count,
        extra_css=['world-category'],
        use_base_css=True,
        gm_mode=gm_mode,
        generator_name='utilities/world/generate_all_world_html.py',
    )
...
generator = make_generator(
    "world-all",
    "Batch-generate HTML for all world docs and write index",
    main,
)
