...
- **Act on process feedback immediately.** When the user mentions a process improvement, flags an error of assumption, or identifies a recurring execution problem, add it to `CLAUDE.md` or `bd remember` in the same turn — don't defer. "Going forward I'll..." is meaningless without a memory write.
- **CLI commands: one block per command, no inline comments.** Shell comments on the same line as a command cause parse errors when users copy a block verbatim. Each command gets its own fenced block; explanatory text goes outside the fence as prose.
- **Interactive location/route editing:** Use `python utilities/devserver.py` instead of `python -m http.server`. The devserver serves `docs/` at `localhost:8000/` and the map workshop at `localhost:8000/workshop`, and exposes write-back endpoints for coordinate updates and rebuilds.
## Morning Report
