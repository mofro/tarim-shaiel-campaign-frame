...
PUBLIC_SECTION_MARKERS = [
    ...
    '## Stats',  # Equipment and creature stats
]
...
ATTRIBUTE_MAPPINGS = {
    ...
    # Equipment attributes (Tier 1 weapons & armor)
    'weapon_tier': ('Tier', 'number', False),
    'weapon_type': ('Weapon Type', 'text', False),
    'weapon_damage_type': ('Damage Type', 'text', False),
    'weapon_trait': ('Trait', 'text', False),
    'weapon_range': ('Range', 'text', False),
    'weapon_damage': ('Damage', 'text', False),
    'weapon_burden': ('Burden', 'text', False),
    'armor_tier': ('Tier', 'number', False),
    'armor_thresholds': ('Thresholds', 'text', False),
    'armor_score': ('Score', 'number', False),
}
...
class KankaAPI:
    ...
    def _rate_limit_wait(self):
        """Respect Kanka's rate limits (30 requests/minute = 2 second delay)"""
        time.sleep(2.0)
...
class ContentRouter:
    ...
    def split_markdown_content(markdown_body: str) -> Tuple[str, Dict[str, str], List[Dict]]:
        """
        Split markdown into public content, GM sections, and daggerheart stat blocks.
        
        Returns:
            (public_content, gm_sections_dict, daggerheart_blocks)
            where gm_sections_dict = {section_name: content}
            and daggerheart_blocks = [parsed stat blocks]
        """
        # Extract daggerheart blocks BEFORE any other processing
        ...
        if dh_blocks:
            stat_html_blocks = [render_statblock_html(block) for block in dh_blocks]
            stat_html = '\n\n'.join(stat_html_blocks)
...
class ObsidianKankaSync:
    ...
    def sync_attributes(self, entity_id: int, metadata: Dict, dh_attrs: List[Dict] = None, dry_run: bool = False):
        """Sync frontmatter fields and daggerheart attributes to Kanka"""
        if dry_run:
            return
        ...
        # Sync frontmatter attributes
        for obs_field, (kanka_name, attr_type, is_private) in ATTRIBUTE_MAPPINGS.items():
            # Skip dh_ prefixed fields - those come from daggerheart blocks
            if obs_field.startswith('dh_'):
                continue
        ...
        # Sync daggerheart attributes if present
        if dh_attrs:
    ...
    def sync_file(self, file_path: Path, dry_run: bool = False) -> bool:
        """Sync single file with enhanced routing and daggerheart support"""
        try:
            # Parse file
            ...
            # Split content and extract daggerheart blocks
            public_content, gm_sections, dh_blocks = self.router.split_markdown_content(post.content)
            
            # Convert daggerheart blocks to attributes
            dh_attrs = []
            if dh_blocks:
                for block in dh_blocks:
                    dh_attrs.extend(daggerheart_to_attributes(block))
            
            # Convert public content to HTML (stat blocks already rendered in split_markdown_content)
            ...
            if dry_run:
                ...
                dh_count = len(dh_attrs)
                logging.info(
                    f"[DRY RUN] {file_path.name} → {entity_type} "
                    f"(private={metadata['is_private']}, {attr_count} attrs, {dh_count} DH attrs, {gm_count} GM posts)"
                )
            ...
            # Sync attributes (both frontmatter and daggerheart)
            self.sync_attributes(entity_id, metadata, dh_attrs, dry_run)
            ...
            # Log daggerheart stats if present
            if dh_attrs:
                logging.info(f"  └─ {len(dh_attrs)} Daggerheart attributes synced")
...
def main():
    parser = argparse.ArgumentParser(description='Obsidian → Kanka Sync (Enhanced v2.1 - Daggerheart)')
    parser.add_argument('--config', default='kanka-sync-config-test.yaml', help='Config file')
    ...
    if args.test_connection:
        ...
        print("Features: Content splitting, attributes, GM posts, Daggerheart parsing\n")
